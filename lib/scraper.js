...
export async function fetchLeibenGesamtTable() {
  ...
  // 1) Wait for the tabs to render, then click the "Gesamt" tab
  await page.waitForSelector('.nav-tabs a[href="#gesamt"]');
  await page.click('.nav-tabs a[href="#gesamt"]');

  // 2) Wait for the Gesamt table to become visible
  await page.waitForSelector('#gesamt table', { visible: true });

  // 3) Scrape everything
  const rows = await page.evaluate(() => {
    const table = document.querySelector('#gesamt table');
    if (!table) return [];

    // grab headers from the thead
  ...
  await browser.close();
  console.log(`✅ Scraped ${rows.length} Gesamt rows:`, rows);
  return rows;
}
