...
export async function fetchLeibenTable() {
  ...
  // Wait only for DOMContentLoaded to avoid hanging on networkidle
  await page.goto(
  ...
  const rows = await page.evaluate(() => {
    // find the <table> under the #gesamt pane (even if hidden)
    ...
    // map each row to an object keyed by header
