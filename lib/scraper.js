...
export async function fetchLeibenGamePlan() {
  ...
  const games = await page.$$eval(
    ...
    containers => containers.map(node => {
      ...
      const {formattedDate, formattedTime} = formatForSupabase(dateText,timeText);
...
export function formatForSupabase(dateText, timeText) {
  // 1) Extract day and month from "Fr. 19.07."
  //    - Remove weekday prefix and trailing dot
  //    - "19.07."
