...
export async function fetchLeibenTable() {
  ...
  // wait for the section that holds the Gesamt table
  await page.waitForSelector('section.app-vhp3_tabelle table.widget-table');

  const rows = await page.evaluate(() => {
    // select the table inside the "app-vhp3_tabelle" panel
    const table = document.querySelector('section.app-vhp3_tabelle table.widget-table');
    if (!table) return [];

    // map over each <tr> in its <tbody>
    return Array.from(table.tBodies[0].rows).map(tr => {
      const cells = Array.from(tr.cells).map(td => td.textContent.trim());
      return {
        Rang:           cells[0],
        // the "u-widget_visible-table--sm" column is the club name cell at index 2
        Mannschaft:     cells[2],
        Spiele:         cells[4],
        Siege:          cells[6],
